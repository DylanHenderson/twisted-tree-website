...
### 20 November 2025
* Added support for product IDs to the inventory updater (instead of barcodes)
* Fixed an error where Shopify bundles did not always display correctly.

### 09 November 2025
* Added a setting to prevent packer modal from displaying and instead use previous packer on the same devices
* Added language override options to the internal packing slip.
* Added a pre-start screen to checking multiple orders to adjust the sorting before checking.
* Fixed a bug where sometimes you couldn't open a Shopify order from mobile devices by clicking the order name on the orders screen
* Improved autofocus behavior on inputs to mostly use the global input detection and not the input bars

### 05 November 2025
* Allow checking multiple orders in a row by selecting them from the orders table

### 26 October 2025
* Allow using default product variants for bin location metafields
* Display custom metafields on the inventory updater

### 23 October 2025
* Packing slip and picking list improvements. Use Shopify print action.

### 16 October 2025
* Singapore local database for faster app usage in the region

### 09 October 2025
* Global input detection, having the input bar selected is no longer required to scan in barcodes with scanners in keyboard mode

### 06 October 2025
* Refresh button added to orders table
* Fixed a bug where product titles were being truncated.

### 30 September 2025
* Performance improvements. Order data should load in most regions about 30% faster.
* Input fields no longer need to be in focus when scanning order or product Barcodes.

### 15 September 2025
* Fixed a bug where order numbers prefixed by an empty space were not processed correctly

### 14 September 2025
* Improved variant bin location support
* Improved Bundles.app integration options.

### 26 August 2025
* Fixed a bug where some settings combinations were not fetching all items on very large orders

### 17 August 2025
* Added support for variant metafields
* IOS sound output fixes
* Additional barcodes minor bug fixes
* Added support for visual notifications on successful scan.
* Settings structure improvements
* Added support for secondary sort for picking lists as well as group by order number support.
* Added pagination at the top of the page for the orders page for mobile devices for ease of use.
* Fixed a bug where bundle.app bundles were not showing scanned items correctly.

### 08 August 2025
* Bin locations migrated to list based metafields
* Bin location filtering added on the metafields page
* Added a fallback when scanning tracking numbers to use order number when tracking number is not found

### 05 July 2025
* Added support for non-latin characters in packing slips
* Added sort by product type option to both picking lists and order checking.
